...
def bbox_overlaps_per_image(anchors, boxes, branch='frcnn'):
    if branch=='rpn':
...
'''
def bbox_overlaps_batch(anchors, boxes, batch_size, branch='frcnn'):
    bbox_overlaps_batch=[]
    for i in range(batch_size):
        bbox_overlaps=bbox_overlaps_per_image(anchors, boxes[i], branch) 
        bbox_overlaps_batch.append(bbox_overlaps)
    return bbox_overlaps_batch
'''
def bbox_overlaps_batch(anchors, boxes, num_boxes, branch='frcnn'):
    bbox_overlaps_batch=[]
    start=0
    for box_size in num_boxes:
        left=start
        right=start+box_size
        bbox_overlaps=bbox_overlaps_per_image(np.vstack(anchors[left:right]), boxes[left:right], branch) 
        bbox_overlaps_batch.append(bbox_overlaps)
        start+=box_size
    return bbox_overlaps_batch
...
def bbox_transform_inv(boxes, bbox_deltas):
    ...
    return pred_boxes.astype(np.int32)